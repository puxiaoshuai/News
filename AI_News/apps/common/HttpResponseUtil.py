...
def generate_response(data=None, message=None, code=ResponseCode.CODE_SUCCESS):
    return {
        'message': message if message is not None else ResponseCode.msg.get(code, ResponseCode.msg[
            ResponseCode.CODE_SERVER_ERROE]),
        'code': code,
        'data': data
    }


def create_list_msg(data,total, page, page_size):
    return {
        'total_num': total,
        'page': page,
        'page_size': page_size,
